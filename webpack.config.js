...
function getEnvFileName(env) {
	...
	switch (true) {
		case !!env.test: return '.env.test';
		case !!env.local: return '.env.local';
		case !!env.development: return '.env.development';
		case !!env.staging: return '.env.staging';
		case !!env.production: return '.env.production';
		default: return '.env.production';
	}
}
...
module.exports = (env, options) => {
	...
	const devServer = options.https ? { ...defaultDevServer, https } : defaultDevServer;
	...
	return {
		...
		target,
		devtool: options.mode === 'production' || !!env.production ? false : 'source-map',
		entry: './src/index.js',
		output: {
