...
function GettingStarted({ location, user }) {
	if (user?.status === gettingStartedStatuses.completedGettingStarted
		&& user.isApprovedByAdmin
		&& location.pathname !== RETAIL_HUB_REGISTRATION_APPROVAL) {
		if (user.role === retailerCompany
			|| user.role === retailerBrand
			|| user.role === retailerConsultant
			|| user.role === retailerServiceProvider
			|| user.role === retailerVentureCapital) return <Redirect to={RETAIL_HUB_REGISTRATION_APPROVAL}/>;

		return <Redirect to={Routes.HOME}/>;
	}

	const isRegistrationApprovalPage = location?.pathname === RETAIL_HUB_REGISTRATION_APPROVAL;
	...
	return (
		<div className='getting-started-container'>
			...
			<div className={cn('form-container', { 'container-centered': isRegistrationApprovalPage })}>
				<Switch>
					<ProtectedRoute
...
}, null, null, { pure: false })(memo(withRouter(GettingStarted)));
