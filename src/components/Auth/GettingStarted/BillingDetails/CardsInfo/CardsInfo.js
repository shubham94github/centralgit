...
const CardsInfo = ({
	...
	const [isDeletePaymentMethodModal, setIsDeletePaymentMethodModal] = useState(false);
	...
	const toggleDeleteConfirmModal = methodId => {
		const selectedCard = cards.find(card => card.paymentMethodId === methodId);
	...
	const handleDetachPaymentMethod = () => {
		if ((!isLastCard && !isGettingStarted) && idDeletePaymentMethod === defaultPaymentMethodId) return;
	...
	return (<>
		{cards.map(({ brand, last4, paymentMethodId }, idx) => {
			...
			const deleteCard = () => {
				if ((!isLastCard && !isGettingStarted) && paymentMethodId === defaultPaymentMethodId) return;
						...
						{!isGettingStarted
							&& <>
								...
								<div
									...
									onClick={deleteCard}
								>
									{(isLastCard || paymentMethodId !== defaultPaymentMethodId)
										&& trashIcon}
								</div>
								<div
