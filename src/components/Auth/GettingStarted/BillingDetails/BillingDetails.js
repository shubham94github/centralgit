...
function BillingDetails({
	...
	const subscriptionEndsAt = dateFormatCorrection(new Date(user[propName].stripePaymentSettings?.nextPaymentDate),
		'MMM d, yyyy' ) || '...';
	const location = useLocation();
	const defaultValues = schema.default();
	const classContainer = cn('billing-details-container', { 'container-modal': isAdmin });
	...
	const handleCopyCompanyInfo = () => {
		const { city, companyLegalName, country: { id: countryId } } = client;
