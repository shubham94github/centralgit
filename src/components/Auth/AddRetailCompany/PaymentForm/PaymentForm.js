...
const PaymentForm = ({
	...
	const totalPrice = (currentPaymentPlan?.price?.unitAmountWithDiscount >= 0
	...
	const isChangeSubscription = location.pathname === Routes.SUBSCRIPTION.UPDATE_CARD || isUpdating;
	const isRegistration = location.pathname.includes('/auth/sign-up/');

	const formTitle = !isModal && (!isChangeSubscription ? 'Add a Payment' : 'Update your payment information');
	...
	const onSubmit = async ({ name, email }) => {
		...
		const { paymentMethod: { id } } = await stripe.createPaymentMethod({
			type: 'card',
			card: cardElement,
			'billing_details': {
				name,
		...
		if (!isChangeSubscription || !!stripeSubscriptionId)
			attachPaymentMethod(id, !isModal && !stripeSubscriptionId, !!stripeSubscriptionId, stripeSubscriptionId);
		...
		} else if (isUnpaidUser)
			recreatePaymentCard(currentPaymentPlan?.id, id);
		 else updatePaymentCard(id);

		if (isModal) toggleModal();
	};

	const handleChangeCardNumber = ({ complete }) => setIsCardNumberFilled(complete);
	...
	const isSubmissionAvailable = isEmpty(errors)
		&& isCardNumberFilled
		&& isAgreementAccepted
		&& isCVCFilled
		&& isExpiryFilled;
	...
	return (
		<>
			<FormWrapper className={`add-retail-company-container ${className}`} onClick={stopPropagation}>
				<Row>
					<Col className='text-center'>
						<P16 className='forms-text-color' bold>{formTitle}</P16>
					</Col>
				</Row>
				...
				<Row>
					<Col
						className='form-field-wrapper mb-3'
						xs={12}
						sm={6}
					>
						<TextInput
				...
				<Row>
					<Col className='card-field-wrapper'>
						<CardNumberElement
				...
				{!isModal
					&& <>
						<Row className='plan-description'>
							...
							>
								{!!currentPaymentPlan.trial
									? <P14>{`After ${currentPaymentPlan.trial} days`}<b>{` $${getFormattedPrice(userPrice ? (userPrice / 100) : totalPrice)}`}</b></P14>
									: <P14>You will be charged<b>{` $${getFormattedPrice(userPrice ? (userPrice / 100) : totalPrice)}`}</b></P14>
								}
							</Col>
							<Col
								sm={6}
							...
							>
								{((!!isUnpaidUser && !stripeSubscriptionId) || isRegistration)
									&& <P14>
										{`${currentPaymentPlan?.uiName} Plan`}
										{isUnpaidUser
											? <span className='link' onClick={openEditPlanModal}>Change</span>
											: <span className='link' onClick={goPrevPage}>Change</span>
										}
									</P14>
								}
							</Col>
				...
				<GridContainer
					template={isChangeSubscription ? '50% 50%' : '50%'}
					columns={isChangeSubscription ? 2 : 1}
					gap='30px'
					customClassName='form-actions'
				>
...
const mapStateToProps = ({ common: { paymentPlans }, auth }) => {
	...
	const paymentPlanId = currentPaymentPlanId ?? getItemFromStorage('paymentPlan');
	const currentPaymentPlan = paymentPlans
		.find(({ id }) => +paymentPlanId === id) || user?.retailer?.paymentPlan;
	...
	return {
		...
		emailForSignUp: emailForSignUp || getItemFromStorage('signUpEmail') || user?.email,
		user,
		isUnpaidUser: (!isAdmin && !isStartup) && !!authUser?.stripePaymentSettings
			&& !authUser?.stripePaymentSettings?.isSubscriptionPaid
			&& !authUser?.stripePaymentSettings?.isTrial,
		stripeSubscriptionId: (!isAdmin && !isStartup)
			&& authUser?.stripePaymentSettings?.stripeSubscriptionId,
		stripePaymentMethodId: (!isAdmin && !isStartup)
