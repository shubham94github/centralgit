...
import {
  ...
  getPaymentPlans as getCommonPaymentPlans,
...
const ChooseYourPlan = ({
  ...
  useEffect(() => {
    if (isEmpty(paymentPlans) && !isModal) {
      const token = user?.token
        ? user?.token
        : getItemFromStorage("resendVerificationToken", rememberMe);
  ...
  useEffect(() => {
    if (activePaymentPlan) setSelectedPlanId(activePaymentPlan.id);
  ...
  const onSubmit = () => {
    if (!isChangeSubscription) {
      ...
      const chosenPlan = paymentPlans.find(
        (paymentPlan) => paymentPlan.id === selectedPlanId
      );
  ...
  const cancelPlanChoosing = () =>
    history.push(Routes.SUBSCRIPTION.UPDATE_CARD);
  ...
  return (
    <FormWrapper className={classNames}>
      ...
      <div className="plans">
        <GridContainer columns={3} gap="10px 20px">
          {columns.map((column, index) => {
            const isEnterprise = column.type === enterpriseType;
            const plans = paymentPlans.filter(
              (paymentPlan) => paymentPlan.planType === column.type
            );
            const features = plansFeatures.filter(
              (plansFeature) => plansFeature[column.featureType]
            );
            ...
            return (
              <PlanCard
                ...
                features={features}
                sendDiscountCode={
                  isEnterprise ? sendEnterpriseCode : sendDiscountCode
                }
