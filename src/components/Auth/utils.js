...
export const redirectToCorrectPageByStatus = user => {
	...
	const isSubscriptionPaid = (!isAdmin && !isStartup)
		&& (!!user[isRetailer ? 'retailer' : 'member'].stripePaymentSettings?.isTrial
			|| (!isStartup && user[isRetailer ? 'retailer' : 'member'].stripePaymentSettings?.isSubscriptionPaid
				&& !user[isRetailer ? 'retailer' : 'member'].stripePaymentSettings.isTrial));
	...
	if (isStartup) {
		switch (status) {
			...
			case accountInfo: return <Redirect to={Routes.AUTH.GETTING_STARTED.COMPANY_INFO}/>;
			case companyInfo: return <Redirect to={Routes.AUTH.GETTING_STARTED.GALLERY}/>;
			case gallery: return <Redirect to={Routes.AUTH.GETTING_STARTED.SECTORS_COMPETENCE}/>;
			case sectorsOfCompetence:return <Redirect to={Routes.AUTH.GETTING_STARTED.AREAS_OF_INTEREST}/>;
			case areasOfInterest: return <Redirect to={Routes.AUTH.GETTING_STARTED.RELATED_TAGS}/>;
			case completedGettingStarted:
				return <Redirect to={Routes.AUTH.GETTING_STARTED.RETAIL_HUB_REGISTRATION_APPROVAL}/>;
			default: return <Redirect to={Routes.AUTH.INDEX}/>;
		}
	}
	...
	if (isRetailer) {
		switch (status) {
			case accountRegistration: return <Redirect to={Routes.AUTH.SIGN_UP.CHOOSE_YOUR_PLAN}/>;
			case chooseYourPlan: return <Redirect to={Routes.AUTH.SIGN_UP.RETAIL_BILLING_DETAILS}/>;
			case paymentMethod: {
				if (user.isVerified) return <Redirect to={Routes.AUTH.GETTING_STARTED.ACCOUNT_INFO}/>;
			...
			case accountInfo: return <Redirect to={Routes.AUTH.GETTING_STARTED.COMPANY_INFO}/>;
			case companyInfo: return <Redirect to={Routes.AUTH.GETTING_STARTED.BILLING_DETAILS}/>;
			case completedGettingStarted: {
