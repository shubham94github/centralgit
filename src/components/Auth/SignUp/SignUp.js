...
const SignUp = ({ isStartup, isMember }) => {
	...
	const cssStyle = {
		...
		backgroundImage: `url(${isStartup ? backgroundStartup : backgroundRetailer})`,
	};
	...
	useEffect(() => {
		if (
			...
			|| location.pathname === Routes.AUTH.SIGN_UP.EMAIL_VERIFICATION_PROCEED_STARTUP
		...
		<section className='sign-up-page-section' style={cssStyle}>
			<div className='sign-up-page-container'>
				...
				<Switch>
					...
					/>
					<Route
						path={Routes.AUTH.SIGN_UP.ADD_EMAIL}
					...
					/>
					<Redirect path={`${Routes.AUTH.SIGN_UP.INDEX}/*`} to={Routes.AUTH.SIGN_IN}/>
				</Switch>
			</div>
		</section>
	);
