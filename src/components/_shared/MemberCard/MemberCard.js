...
const MemberCard = ({
	...
	const handleActivation = () => changeMemberStatus({
			...
			<div className='about'>
				{!!item.position
					&& <Tooltip
						...
						placement='bottom'
						isVisibleTooltip={item.position?.name.length >= maxStringLengthNoEllipsis}
					>
						<P14 className='position'>{userIcon}{item.position?.name}</P14>
					</Tooltip>
				}
				{!!item.department
					&& <Tooltip
						...
						isVisibleTooltip={item.department?.name.length >= maxStringLengthNoEllipsis}
					>
						<P14 className='department'>{bagIcon}{`${item.department.name}`}</P14>
					</Tooltip>
				}
				{!!item.email
					&& <Tooltip
					...
					>
						<P14 className='email'>{emailIcon}{item.email}</P14>
					</Tooltip>
				}
				{(item.country && item.city)
					&& <Tooltip
						placement='bottom'
						message={locationAddress}
						isVisibleTooltip={locationAddress.length >= maxStringLengthNoEllipsis}
					>
						<P14 className='location'>{locationIcon}{locationAddress}</P14>
					</Tooltip>
				}
			</div>
