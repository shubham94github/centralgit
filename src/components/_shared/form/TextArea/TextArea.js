...
const TextArea = ({
	...
	const handleKeyDown = e => {
		...
		if (e.key === 'Enter' && !e.shiftKey)
			document.querySelector('textarea.textarea').style = 'height: 28px; overflow-y: hidden;';
	};
	...
	const customPlaceholder = (withTopPlaceholder && !isFocus)
		? placeholder
		: !withTopPlaceholder
			? placeholder
			: '';
	...
	return (
		<div className={styles}>
			{!withTopPlaceholder && label
				&& <label className={isFixedHeightLabel ? 'fixed-height-label' : ''} htmlFor={id}>
					{label}
				</label>
			}
			{isEditText
