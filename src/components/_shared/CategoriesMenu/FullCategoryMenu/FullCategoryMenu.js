...
const FullCategoryMenu = ({
  ...
  useEffect(() => {
    if (categoriesIds[0] && !categoriesIds[1]) selectedFirstCategoryNode.current.scrollIntoView({ block: 'center' })
  }, [categoriesIds])
  ...
  return (
    <div ref={fullMenuNode} className={wrapperClasses}>
      <div onClick={toggleFullMenuHandler} className='menu-toggle'>
        {categoriesHomeIcon}
        <span className={textCategoriesClasses}>{isBrowseFullMenu ? 'All Categories' : 'Categories'}</span>
      </div>
      ...
      {hideSomeFirstLevelCategories(categories).map(({ id, logo, name, items: secondLevel }) => {
        ...
        const expandSecondLevelCategory = () => {
          if (categoriesIds[0] === id) handleOpenCategories([])
        ...
        const menuIcon = isOpenSecondCategory ? categoriesMinusIcon(colorForHover) : categoriesPlusIcon(colorForHover)
          ...
          <div key={id}>
            <div
              ...
              <span onClick={onClickHandler} className='category-item clickable'>
                {!!logo?.svg && <img src={logo.svg} alt='icon' />}
                <span
                ...
                {!!secondLevel &&
                  secondLevel.map(({ name, items: thirdLevel, id }) => {
                    ...
                    const expandThirdLevelCategory = () => {
                      if (categoriesIds[1] === id) handleOpenCategories([categoriesIds[0]])
                      else handleOpenCategories([categoriesIds[0], id])
                    }
                    const classes = cn('categories-third-level-wrapper', { hide: !isOpenThirdCategory })
                    const onClickHandler = () => onCategoryClickHandler([...categoriesIds, id])
                    const colorForHover = idSHoverCategory.includes(id) ? colors.grass50 : colors.white
                        ...
                        {!!thirdLevel && (
                          <div className={classes}>
                            {thirdLevel.map(({ name, id }) => {
                              ...
                              return (
                                <div key={id} className='categories-item-third-level'>
                                  <span
                                    onClick={onClickHandler}
                                    className='full-menu-name'
                                    style={{ color: colorForHover }}
