...
const ItemCategory = ({ icon, isLoading, category }) => {
  const { name, sameItems } = category

  return (
    <div className='content-item-category'>
      {!icon && isLoading ? <LoadingOverlay classNames='loader-slide' /> : !!icon && <img src={icon} alt='icon' />}
      <P16 className='title-slide'>
        {name}
        {sameItems.length > 1 && <>&nbsp; ({sameItems.length})</>}
      </P16>
    </div>
  )
}
