...
const CategoriesSwiper = ({ selectedCategories, categories, isLoading, getCategories, isWithoutTooltip, id }) => {
      ...
      <div className='categories-swiper-wrapper'>
        ...
        <Swiper
          ...
          {sortedCategories.map((category, i) => {
            const currentCategory = categories.find(categoryItem => categoryItem.name === category.name)
            const icon = isAreas ? currentCategory?.areasLogo?.svg : currentCategory?.logo?.svg
            console.log('icon ->', icon, currentCategory, categories, category)
            return (
              <SwiperSlide key={category.id + i + category.name}>
                {isWithoutTooltip ? (
                  <ItemCategory icon={icon} isLoading={isLoading} category={category} />
                ) : (
                  <Tooltip
                    trigger={['hover', 'focus']}
                    message={
