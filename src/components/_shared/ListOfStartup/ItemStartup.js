...
const ItemStartup = ({
	...
	const getYearFounded = useMemo(() => new Date(foundedAt).getFullYear(), [foundedAt]);
	const isFullNameFilled = !!(user?.firstName && user?.lastName);

	const handleOpenProfile = useCallback(() => {
		if (isAdmin) history.push(`/admin-panel/profile/startup/${user.id}`);
		else if (!isTrial || trialData?.isTrialProfile)
			history.push(`/profile/startup/${id}`);
		else setWarningOfProfileRestriction();
	}, [history, id, isTrial, trialData, setWarningOfProfileRestriction, isAdmin, user]);
					...
					<div className='content-startup'>
						<div className='d-flex flex-row flex-nowrap justify-content-between align-items-center pb-2'>
							<div className='d-flex flex-row flex-nowrap align-items-center'>
								...
								{!isAdmin && isFullNameFilled && !isDemoStartup && isEnableMailing
										&& <Tooltip
											placement='bottom-end'
											message={
												<P12>{profileStartupText}</P12>
											}
										>
											<div
						...
						{!!solutionProductsServices && (
							<>
								<P12 className='mt-2' bold>Solutions, products and services: </P12>
								<div className='company-description'>
									<Markdown options={{ forceInline: true, wrapper: 'div' }}>
							...
							!isEmpty(categories)
								&& <div className='mt-2 d-flex'>
									<div className='sector-size'>
										<P12 className='text-bold sector-size'>Sector of competence: </P12>
									</div>
									<div>
						...
						}
						{
							!isEmpty(areasOfInterest)
								&& <div className='mt-2 d-flex'>
									<div className='sector-size'>
										<P12 className='text-bold sector-size'>Areas of interest: </P12>
									</div>
									<div>
										<P12 className='sectors-style'>
											{
...
export default memo(ItemStartup);
