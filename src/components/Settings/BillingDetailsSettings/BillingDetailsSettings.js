...
const BillingDetailsHOC = ({
  ...
  const onSubmit = values => {
    ...
    updateBillingDetails({
      ...
      paymentMethod: creditPayment ? 'CREDIT' : 'WEB',
      paymentPlanId
    })
  }
  ...
  return (
    <div>
      <BillingDetails
        ...
        toggleCreditPayment={toggleCreditPayment}
        schema={schema}
        onSubmit={onSubmit}
...
const mapStateToProps = ({
  auth: { cardsInfo, defaultPaymentMethodId, user },
  common: { countries, paymentPlans, isLoading: isLoadingCommon },
  settings: { isLoading }
}) => {
  const localUser = getItemFromStorage('user')
  const { retailer, role, startup, status, city, country } = user || localUser
