...
const CompanyMembersHOC = ({
  ...
  const toggleAddModal = () =>
    setIsShowAddMemberModal((prevState) => !prevState);

  const toggleEditModal = () =>
    setIsShowEditMemberModal((prevState) => !prevState);
  ...
  const onSubmit = ({ memberData, setFormError, onClose }) =>
    addNewMember({ memberData, setFormError, onClose });
  ...
  const onEditMember = ({ memberData, onClose, setFormError }) =>
    editMember({
      memberData: {
        ...memberData,
  ...
  const onDeleteMember = () =>
    deleteMember({ memberId: memberIdForDelete, onClose: toggleConfirm });
...
export default connect(
  ({
    common: { countries, departments, positions },
    settings: { isLoading, members },
    auth,
  }) => {
