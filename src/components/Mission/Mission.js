...
import { getStartupsOptions, getCountries, getCategories, sendMission } from '@ducks/common/actions';
...
const Mission = ({
	...
	const toggleAreasOfInterestModal = () => setIsAreasOfInterest(prevState => !prevState);
	...
	const handleRemoveAreasOfInterestItem = id =>
		setSelectedAreasOfInterest(prevState => prevState.filter(area => area.id !== id));
			...
			<div className='mission-container'>
				<div className='title mb-40'>
					...
					<H1 bold className='text-darkblue'>
						We are  always ready for a mission!
					</H1>
				</div>
				...
				<GridContainer gap='10px 30px' customClassName='mb-40'>
					...
					<P14>Please add a Title for the Search</P14>
					<TextInput
						type='text'
						name='searchTitle'
					...
					/>
					{!!errors.searchTitle && <P12 className='warning-text'>{errors.searchTitle.message}</P12>}
				</GridContainer>
				<GridContainer gap='10px 30px' customClassName='mb-40'>
					<P16 bold>Description</P16>
					<P14>Please add as much information about what you are looking for</P14>
					<TextArea
					...
					/>
					{!!errors.description && <P12 className='warning-text'>{errors.description.message}</P12>}
				</GridContainer>
				<GridContainer
					gap='20px 30px'
					columns={2}
				...
				>
					<GridContainer gap='10px 30px' >
						...
						<P14>What is your approx timeline for implementation</P14>
						<Select
						...
						{!!errors.implementationTime
							&& <P12 className='warning-text'>{errors.implementationTime.message}</P12>
						}
					</GridContainer>
					<GridContainer gap='10px 30px'>
				...
				<GridContainer gap='10px 30px' customClassName='mb-20'>
					...
					<Select
						...
						register={register}
						placeholder='Please select Operating Location(s)'
						isMulti={true}
						isClearable={true}
						isCreatable={true}
						control={control}
				...
				<GridContainer gap='10px 30px' customClassName='mb-20'>
					...
					<P14>Please list out any Startups that you want to exclude from the search</P14>
					<Select
				...
				<GridContainer gap='10px 30px' customClassName='mb-40'>
					...
					<P14>
						Please&nbsp;<span className='blue-link' onClick={toggleAreasOfInterestModal}>select</span>&nbsp;
						the Categories that you would like us to search
					</P14>
					<ChosenFieldLabels
				...
				<GridContainer columns={2} template='270px 270px'
