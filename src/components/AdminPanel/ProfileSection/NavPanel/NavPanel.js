...
const NavPanel = ({ navbarLinks, isStartup }) => {
	...
	const navbarLinksForRole = useMemo(() => navbarLinks.filter(({ roles }) =>
		roles.includes(isStartup ? 'startup' : 'user')), [navbarLinks, isStartup]);

	const [selectedId, setSelectedId] = useState(navbarLinks.find(link => link.path === location.pathname).id);

	useEffect(() => setSelectedId(navbarLinks.find(link => link.path === location.pathname).id),
		[navbarLinks, location]);
