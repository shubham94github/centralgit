...
const { retailerCompany, retailerBrand, retailerConsultant, retailerServiceProvider, retailerVentureCapital } =
  enums.userRoles
...
const ShortInfo = ({
  ...
  const isCompanyRetailer =
    role === retailerCompany ||
    role === retailerBrand ||
    role === retailerConsultant ||
    role === retailerServiceProvider ||
    role === retailerVentureCapital
  ...
  const truncateCompanyName = name => {
    if (name?.length > maxLengthName) {
      ...
      return (
        <Tooltip placement='bottom' message={<P12>{name}</P12>}>
          <span className='pb-1'>{truncateName}...</span>
        </Tooltip>
      )
    }
    return name
      ...
      <Row className='short-info-wrapper g-3'>
        <Col xs={isApprovedByAdmin ? 8 : 5} className='d-flex'>
          <div className='img-profile-wrapper me-3'>
            <Tooltip
              ...
              {isEditPermission ? (
                <span onClick={toggleLogoModal} className='startup-logo'>
                  <Avatar logo={logo60 || {}} className='avatar-for-profile' />
                  <span className='startup-logo-icon'>{editIcon}</span>
                </span>
              ) : (
                <Avatar logo={logo60 || {}} className='avatar-for-profile' />
              )}
          ...
          <div>
            <div className='pb-1 d-flex flex-column'>
              <H3 className='text__darkblue text-start mb-1' bold>
                ...
                <>
                  {isVisibleChatIcon && <span className='ms-2 clickable'>{chatIcon}</span>}
                  <Tooltip
                    isVisibleTooltip={true}
                    placement='right'
                    message={<P12>Send direct notification</P12>}
            ...
            <div>
              <P12 className='word-wrap'>
                ...
                {country.name}
                {city && <>, {city}</>}
                {isStartup && <>&#44;&nbsp;Founded&nbsp;in&nbsp;{getYearFounded(foundedAt)}</>}
                <br />
                {isStartup && (
                  <span className='blue-link truncate mx-3' onClick={openWebsiteLink} rel='noreferrer'>
                    {urlOfCompanyWebsite}
                  </span>
                )}
              </P12>
        ...
        <Col className='d-flex justify-content-end'>
              ...
              {isApprovePermission &&
                (isStartup ? (
                  <Tooltip
                    ...
                    <div className='pb-2'>
                      <PrimaryButton text='Approve' onClick={onClickApprove} disabled={!isCompletedGettingStarted} />
                    </div>
                  </Tooltip>
                ) : (
                  <PrimaryButton text='Approve' onClick={onClickApprove} disabled={!isCompletedGettingStarted} />
          ...
          <div>
            ...
            {status && <S12>Status: {status}</S12>}
            {isChangeStatusPermission && (
              <div className='d-flex align-items-center'>
                <P12 className='pe-2 pb-2'>Account status</P12>
                <SliderCheckbox id={'isBlocked'} checked={!isBlocked} onChange={handleActivation} />
              </div>
            )}
          </div>
        </Col>
...
export default connect(
  ({ admin: { profile, isAdminNotificationSending }, auth }) => {
    ...
    const { isApprovedByAdmin, onlineStatus, isBlocked, status, id, approvedByAdminAt, role } = profile
