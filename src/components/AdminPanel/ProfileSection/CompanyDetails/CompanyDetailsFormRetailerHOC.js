...
const CompanyDetailsFormRetailerHOC = ({
  ...
  const selectedDefaultValues = useMemo(
    () => getDefaultValuesRetailer(profile, countries, userType),
...
const mapStateToProps = ({ admin, common: { countries, tags, isLoading }, auth: { topLevelCategories } }) => {
  const {
    profile: { retailer, role, userType }
  } = admin
