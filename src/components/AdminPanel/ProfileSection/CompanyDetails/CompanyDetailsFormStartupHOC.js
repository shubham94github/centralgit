...
const CompanyDetailsFormStartupHOC = ({
  ...
  const revalidateTargetMarket = (setValue, targetMarket) => {
    if (!targetMarket?.length) setValue('targetMarket', null, { shouldValidate: true })
  }
...
const mapStateToProps = ({ admin, common: { countries, platformPartners, isLoading } }) => {
  const {
    profile: { startup, role }
  } = admin
