      ...
      <Row className='company-detail-wrapper'>
        <Col sm={12} md={4} className='mb-4'>
          ...
          <div className='company-short-info-wrapper'>
            {phoneNumber && (
              <P12 className='pb-1'>
                <span className='text__bold'>Phone: </span>
                {phoneNumber}
              </P12>
            )}
            {owner && (
              <P12 className='pb-1'>
                <span className='text__bold'>Founder: </span>
                {owner}
              </P12>
            )}
            {companyType && (
              <P12 className='pb-1'>
                <span className='text__bold'>Company type: </span>
                {companyType}
              </P12>
            )}
            {businessModel && (
              <P12 className='pb-1'>
                <span className='text__bold'>Business model: </span>
                {businessModel}
              </P12>
            )}
            {companyStatus && (
              <P12 className='pb-1'>
                <span className='text__bold'>Company status: </span>
                {companyStatus}
              </P12>
            )}
            {!!targetMarket.length && (
            ...
            {!isStartup && urlOfCompanyWebsite && (
              <P12 className='pb-1 d-flex'>
                ...
                <span className='blue-link truncate' onClick={openCompanyWebsite} rel='noreferrer'>
                  {urlOfCompanyWebsite}
                </span>
              </P12>
            )}
            {linkedInCompanyPage && (
              <P12 className='pb-1 d-flex'>
                ...
                <span className='blue-link truncate' onClick={openLinkedInPage} rel='noreferrer'>
                  {linkedInCompanyPage}
                </span>
              </P12>
            )}
              ...
              <>
                ...
                <Tooltip
                  ...
                  message={
                    <P12>
                      {clientsList.map((exampleItem, index) => (
                        <span key={index + exampleItem + index}>
                          {index !== 0 && ', '}
                          {exampleItem}
                        </span>
                      ))}
                  ...
                  <div className='d-inline-block'>
                    <P12 className='pb-1'>
                      <span className='text__bold number-of-clients'>Number of Clients:&nbsp;</span>
                      {numberOfClients}
                    </P12>
                  </div>
                </Tooltip>
        ...
        <Col sm={12} md={8}>
          <P14 className='flex-section edit-icon-container' bold>
            Company Description:
            {isEditPermission && (
              <span onClick={toggleCompanyDetailsEditModal} className='clickable edit-icon'>
          ...
          {isStartup && (
            <>
              ...
              {solutionProductsServices && <TextToggle text={solutionProductsServices} />}
              <P16 className='flex-section pt-4 pb-3 edit-icon-container' bold>
              ...
              {!isEmpty(categories) && <CategoriesButtons categories={categories} />}
              <P16 className='flex-section pt-4 pb-3 edit-icon-container' bold>
                Areas of interest
                {isEditPermission && (
              ...
              {!isEmpty(areasOfInterest) && <CategoriesButtons categories={areasOfInterest} />}
            </>
          )}
          {!isStartup && (
            <>
              ...
              {!isEmpty(companySectors) && <CategoriesButtons categories={companySectors} />}
            </>
          )}
          <P16 className='flex-section pt-4 pb-3 edit-icon-container' bold>
            Associated tags
