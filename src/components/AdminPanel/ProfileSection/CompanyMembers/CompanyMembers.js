...
import { getCountries, getPositions, getDepartments, setSnackbar } from '@ducks/common/actions';
import enums from '@constants/enums';
import { addMemberSchema, editMemberSchema } from '../../../Settings/CompanyMembersSettings/schema';
import { changeMemberStatus, createNewMember, downloadMemberAvatar, editMember, removeMember } from '@api/adminApi';
...
import { addMemberTitle, editMemberTitle } from '@components/AdminPanel/ProfileSection/CompanyMembers/constants';
import { successAddedMember, successDeletedMember, successEditedMember } from '@ducks/settings/sagas';
...
const CompanyMembers = ({
	...
	const handleSaveMember = async ({ memberData, setFormError, setIsLoading }) => {
		try {
			if (setIsLoading) setIsLoading(true);

			const saveMemberRequest = !!memberData.userId ? editMember : createNewMember;
	...
	const handleChangeMemberStatus = async ({ memberId, setFormError, setIsLoading, isBlocked }) => {
		try {
			if (setIsLoading) setIsLoading(true);
	...
	const openDeletingConfirmationModal = memberId => setDeletingMemberId(memberId);
...
export default connect(({ common: { isLoading, countries, positions, departments }, admin: { profile }, auth }) => {
