  ...
  return [
    {
      ...
      cell: ({ isBlocked, isApprovedByAdmin, id, status }) => {
        const handleApprove = () =>
          approveUser({ ids: [id], type: verifyUserTypes.startup });
        const handleActivation = () =>
          handleStartupActivation({ ids: [id], isBlocked });
        const outerProps = {
          id,
          isApprovedByAdmin,
          isBlocked,
    ...
    },
    {
      ...
      cell: (user) => {
        ...
        return (
          <div onClick={handleOpenProfile} style={{ cursor: "pointer" }}>
            {getCompanyIcon(
              user.startup.logo30,
    ...
    },
    {
      ...
      cell: (user) => {
        ...
        return (
          <Tooltip
            isVisibleTooltip={
              user.startup.companyShortName.length > maxStringLengths.shortName
            }
            placement={"bottom"}
    ...
    },
    {
      ...
      cell: (user) => {
        ...
        return (
          <Tooltip
            isVisibleTooltip={
              user.startup?.owner.length > maxStringLengths.founder
            }
            placement={"bottom"}
            message={user.startup.owner}
          >
            <div style={{ cursor: "pointer" }} className="truncate-brand">
              {user.startup.owner}
            </div>
          </Tooltip>
        );
        ...
        return (
          areasOfInterest && (
            <Tooltip
              ...
              message={
                <P12 className="parent-paths">
                  {combineCategoriesByName(areasOfInterest).map(
                    (areaOfInterest, i) => {
                      return (
                        <div key={i}>
    ...
    },
    {
      ...
      cell: ({ startup: { rateStars } }) => {
        const starsArray = Array.from({ length: rateStars }, () =>
          Icons.ratingStarIcon(colors.orange40)
        );
    ...
    },
    {
      ...
      width: "200px",
      cell: ({ onlineStatus }) => {
        const lastSeen = onlineStatus?.visitedAt
          ? dateFormattingWithTime(onlineStatus?.visitedAt)
          : "";
