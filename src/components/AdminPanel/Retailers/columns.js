  ...
  return [
    {
      ...
      cell: ({ isBlocked, id, isApprovedByAdmin, role, status }) => {
        const handleApprove = () =>
          approveUser({ ids: [id], type: verifyUserTypes.retailer });
        const handleActivation = () =>
          handleRetailerActivation({ ids: [id], isBlocked });
        const outerProps = {
          id,
          isApprovedByAdmin,
          isBlocked,
      ...
      cell: ({ retailer: { companyShortName }, id }) => {
        ...
        return (
          <Tooltip
            // eslint-disable-next-line react/prop-types
            isVisibleTooltip={
              companyShortName?.length > maxStringLengths.shortName
            }
            placement="bottom"
            ...
            <Tooltip
              ...
              message={
                <P12 className="parent-paths">
                  {combineCategoriesByName(companySectors).map(
                    (companySector, i) => (
                      <div key={i}>
    ...
    },
    {
      ...
      cell: ({
        retailer: {
          stripePaymentSettings: { isSubscriptionPaid },
        },
        status,
      }) => {
        const rule =
    ...
    },
    {
      ...
      cell: ({
        retailer: {
          stripePaymentSettings: { nextPaymentDate },
        },
      }) => {
        // eslint-disable-next-line react/prop-types
        const lastSeen = nextPaymentDate
          ? dateFormattingWithTime(nextPaymentDate)
          : "";
