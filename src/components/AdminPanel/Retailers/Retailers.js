...
const { groupActions, verifyUserTypes } = enums;
...
const Retailers = ({
  ...
  useEffect(
    () => setPageCount(Math.ceil(retailersCount / retailersTableMeta.size)),
    [retailersCount, setPageCount, retailersTableMeta.size]
  );

  useEffect(() => {
    if (isEmpty(retailers)) getRetailers(retailersTableMeta);
  ...
  const onChangeCountOfRecords = (size) =>
    getRetailers({
      ...retailersTableMeta,
      size,
      page: 1,
  ...
  const onSearch = (data) => {
    ...
    const isFieldExist = retailersTableMeta.fields.find(
    ...
    if (!isFieldExist) {
      ...
      getRetailers({
        ...retailersTableMeta,
        fields: [...retailersTableMeta.fields, { ...data }, ...filterParams],
        page: 1,
      });
    } else {
      const index = retailersTableMeta.fields.findIndex(
        (item) => item.fieldName === data.fieldName
      );
      const newReqData = retailersTableMeta.fields.map((item, i) => {
        if (i === index)
          return { fieldName: item.fieldName, filterText: data.filterText };
  ...
  const handleChangeFilter = (filters) => {
    ...
    const oldFields = retailersTableMeta.fields.filter(
...
export default connect(
  (state) => {
    ...
    return {
      ...
      listOfPermissions,
      isRetailersViewProfilePermission:
        listOfPermissions?.isRetailersViewProfilePermission,
      isRetailersApprovePermission:
