...
const SubscriptionPlans = ({
  ...
  const createSubscriptionPlan = (newPaymentPlan, setFormError) =>
    saveNewSubscriptionPlan(newPaymentPlan, closeAddNewSubscriptionPlan, setFormError)
  ...
  const memberGroupsOptions = useMemo(
    () => memberGroups.map(item => ({ label: item.name, value: item.id })),
    [memberGroups]
  )
  ...
  const filteredPlans = useMemo(() => {
    ...
    return subscriptionPlans.filter(item => {
      ...
      const isMemberId =
        !chosenFilters?.memberGroupId?.value || item.memberGroup?.id === +chosenFilters.memberGroupId.value
      const isPrice =
        !chosenFilters?.paymentPlanPriceIds?.value || item.price.id === +chosenFilters.paymentPlanPriceIds.value
  ...
  return (
    <div className='subscription-plans'>
      ...
      <div className='guidelines'>
        {infoIcon}
        <P14>
          Create a Payment Plan based on a User Role. If the selected Plan Type is Enterprise, a generated code will be
          provided to the potential user via email or sms. To access the generated code,
          <br />
          go to&nbsp;
          <S14 className='link' onClick={handleClickEnterprise}>
            Enterprise
          </S14>
          .
        </P14>
      </div>
      <GridContainer gap='15px' columns={2} customClassName='buttons'>
