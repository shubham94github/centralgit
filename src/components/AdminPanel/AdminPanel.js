...
import PaymentReceipts from '@components/AdminPanel/SubscriptionPlans/PaymentReceipts'
...
const AdminPanel = ({ user, menuItems, isCategoryViewPermission, isStartupAnalysisPermission }) => {
  ...
  return (
    <div className='admin-panel-container'>
      <SideBarMenu menuItems={menuItems} userRole={user.role} title='Admin Panel' isExpandableItems={true} />
      <section className={classes}>
        <Switch>
          <Route path={INDEX} render={() => <Redirect to={STARTUPS} />} exact />
          <ProtectedRoute path={STARTUPS} component={Startups} />
          {isStartupAnalysisPermission && <ProtectedRoute path={STARTUP_ANALYSIS} component={StartupAnalysis} />}
          <ProtectedRoute path={RETAILERS} component={Retailers} />
          {isCategoryViewPermission && <ProtectedRoute path={MANAGE_CATEGORIES} component={ManageCategories} />}
          ...
          {isSuperAdmin && (
            <>
              ...
              <ProtectedRoute path={PAYMENT_RECEIPTS} component={PaymentReceipts} />
