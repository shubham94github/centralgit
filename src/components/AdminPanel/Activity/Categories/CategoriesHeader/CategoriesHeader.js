...
const CategoriesHeader = ({
	...
	const dateFrom = dateRange?.length ? dateRange[0] : null;
	const dateTo = dateRange?.length ? dateRange[1] : null;
	...
	return (
		<div className='activity-categories-header'>
			<div className='description'>
				{infoIcon}
				<P14>
					Super Admin can be able to monitor the activities of all admins involving Categories.
				</P14>
			</div>
			...
			<div className={filterClasses}>
				{listOfFilters.map((filter, i) => {
					const { placeholder, property, type } = filter;

					const listOfOptions = property === 'adminRole'
						? authoritiesList
						: permissions.manageCategories.items;
