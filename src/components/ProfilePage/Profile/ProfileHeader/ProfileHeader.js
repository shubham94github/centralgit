...
const ProfileHeader = ({
  ...
  const bookmarkIcon = Icons.bookmarkIcon(isBookmarked ? colors.grass50 : colors.darkblue70)

  const aboutItems = useMemo(
    () => setAboutItems(country, owner, foundedAt, totalFundingAmount, linkedInCompanyPage, urlOfCompanyWebsite),

    [country, owner, foundedAt, totalFundingAmount, linkedInCompanyPage, urlOfCompanyWebsite]
  )

  const isVisibleChatIcon =
    isFullNameFilled && !isStartup && (isRetailer ? !isAuthRoleRetailer : !isDemoStartup && isEnableMailing)
  ...
  const makeAbsouluteUrl = link => (link.indexOf('://') === -1 ? 'http://' + link : link)
      ...
      <div className='image-section'>
        ...
        <div>
          <div className='company-name'>
            ...
            {isVisibleChatIcon && (
              <Tooltip placement='bottom' message={<P12>{isRetailer ? profileUserText : profileStartupText}</P12>}>
            ...
            {!isRetailer && !isStartup && (
              <Tooltip placement='bottom' message={<P12>{isBookmarked ? unBookmarkText : bookmarkText}</P12>}>
                <div className='p-2 clickable' onClick={handleBookmarkClick}>
                  {bookmarkIcon}
                </div>
              </Tooltip>
            )}
            <div className='ps-2 py-2 chat-icon' onClick={handleRetailHubChat}>
              {Icons.chatIconProfile(colors.orange40)}
            </div>
          </div>
          {!!city && !!country?.name ? (
            <div>
          ...
          {!!linkedInCompanyPage && isAbsoluteUrl(linkedInCompanyPage) ? (
            <div>
              <P12 className='word-wrap'>
                <a href={makeAbsouluteUrl(linkedInCompanyPage)} target={`_blank`}>
                  <span className='pe-2'>
          ...
          {!!urlOfCompanyWebsite ? (
            <div>
              <P12 className='word-wrap'>
                <a href={makeAbsouluteUrl(urlOfCompanyWebsite)} target={`_blank`}>
                  <span className='pe-2'>
        ...
        <div className='about-section'>
          <div className='about-panel'>
            {aboutItems.map(({ id, title, value }) => {
              ...
              const isVisibleTooltipOwner = owner?.length > 50 && title.name === 'Founder'
              ...
              return (
                <div className='d-flex' key={id}>
                  <div className='about-item'>
                    ...
                    <Tooltip placement='bottom' isVisibleTooltip={isVisibleTooltipOwner} message={owner}>
...
ProfileHeader.propTypes = {
  ...
  handleRetailHubChat: func.isRequired,
