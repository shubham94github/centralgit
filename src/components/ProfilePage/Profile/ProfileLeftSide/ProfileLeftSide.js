...
const ProfileLeftSide = ({
	...
	const isVisibleTraction = !isRetailer && (!!platformPartners.length || integrationTiming || !!numberOfClients
			|| !!clientsList.length || !!presenceInCountries.length);
	...
	return (
		<div className='left-side-profile'>
			...
			{isRetailer
				&& <div className='blue-link truncate' onClick={openCompanyWebsite}>{urlOfCompanyWebsite}</div>
			}
			{!isRetailer
										...
										<P16 className='text-gray'>
											{
												clientsList.map((exampleItem, index) => (
													<span key={exampleItem+index}>{index !== 0 && ', '}
														{exampleItem}
													</span>
												))
											}
			...
			{!isEmpty(tags) && !videoInterview
				&& <>
					<H1
						className='text-black text-start mt-40 pb-3'
						bold
					>
						Associated tags
					</H1>
