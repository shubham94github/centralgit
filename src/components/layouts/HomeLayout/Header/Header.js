...
import { getNotifications, onNotifications, clearNotifications } from '@ducks/notifications/actions';
...
const { notificationsTooltipText, chatTooltips: { headerText } } = tooltipsText;
...
const Header = ({
	...
	const isSubscriptionPage = location.pathname.includes(Routes.SUBSCRIPTION.INDEX);
	const isPasswordRecovery = !isAuthenticated && location.pathname.includes(Routes.AUTH.PASSWORD_RECOVERY.INDEX);
	...
	const isRetailer = user?.role !== userRoles.startup
	...
	const companyShortNameTruncate = user && companyShortName && companyShortName.length > maxLengthName
		? `${companyShortName.substr(0, maxLengthName)}...`
		: companyShortName;
	const isCompletedStatus = user?.status === enums.gettingStartedStatuses.completedGettingStarted;
	...
	const isEmailVerifyingPage = location.pathname === Routes.AUTH.SIGN_UP.EMAIL_VERIFICATION_PROCEED_STARTUP
	...
	useEffect(() => {
		if (user?.id && isCompletedStatus && isApproved && !isSubscriptionPage && isVerified) getNotifications();
	...
	useEffect(() => {
		if (isAuthenticated && isCompletedStatus && isApproved) {
			...
			if (!newMessagesCounter && isCompletedStatus && !isSubscriptionPage && isVerified)
	...
	useEffect(() => {
		...
		if (isAuthenticated && isCompletedStatus && isApprovedByAdmin
			&& !isSubscriptionPage && isVerified && (isRetailer || (isMember && !user?.member?.isDisabled))
	...
	const successfulHandleSearch = ({ search, category }) => {
		...
		if (location.pathname === Routes.BROWSE_PAGE)
	...
	const handleClick = e => {
		...
		if (location.pathname === Routes.AUTH.SIGN_UP.EMAIL_VERIFICATION_PROCEED_RETAIL
			|| location.pathname === Routes.AUTH.SIGN_UP.EMAIL_VERIFICATION_PROCEED_STARTUP) return;
						...
						<Row>
							<Col className='d-flex justify-content-end'>
								<NavLink className='link' to={Routes.AUTH.SIGN_IN}>
									{logInIcon}
									<S14 className='link__text-margin link__text-white'>Log in</S14>
								</NavLink>
							</Col>
						</Row>
					</Container>
				...
				<div className='home-container'>
					<Row className='justify-content-between'>
						...
						<Col className='d-flex justify-content-end align-items-center'>
							{
								(isRetailer || isMember || isEmailVerifyingPage || isGettingStarted)
									&& <div className={
										cn('nav-links-wrapper',
													...
													return link.tooltipText
														? (
															...
															>
																...
																>
																	{link.icon}
																	<S14 className='link__text-white'>
																		{link.name}
																	</S14>
																</div>
															</Tooltip>
										...
										<CustomDropdown
											...
											button={
												<Tooltip
													...
													message={
														<P12>{notificationsTooltipText}</P12>
													}
												>
													<div className='py-2'>
														<span
										...
										<div className='notification__ml'>
											<CustomDropdown
												...
												<ul>
													...
													{!isAdmin
														&& <li>
															<NavLink className='link' to={Routes.SETTINGS.INDEX}>
																<S16>
																	<span className='pe-3'>
																		{settingsIcon}
																	</span>
...
export default connect(state => {
	...
	return {
		...
		countOfNewNotifications,
		isTrial: (!!user && !isAdmin && !isStartup)
