...
const { AUTH: { GETTING_STARTED: { INDEX, RETAIL_HUB_REGISTRATION_APPROVAL } } } = Routes;
...
const ProtectedRoute = ({
	...
	const securedRouteMatch = () => {
		...
		if (isAuthenticated && user?.status === gettingStartedStatuses.completedGettingStarted
			...
			&& (!user[isRetailer ? 'retailer' : 'member'].stripePaymentSettings?.isSubscriptionPaid
...
}, { setUser, generateListOfPermissions })(withRouter(memo(ProtectedRoute)));
