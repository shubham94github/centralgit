...
const Startups = withSuspense(
  lazy(() => retryLoad(() => import("./Startups")))
);
const NewsFeed = withSuspense(
  lazy(() => retryLoad(() => import("@components/HomePage/NewsFeed")))
);
...
const mapStateToProps = ({
  home: { newStartups, ratedStartups, relatedStartups, isLoading },
  auth: { user },
  profile: storeProfile,
}) => {
  const localStorageUser = getItemFromStorage("user");
  const { profile, feedbacks, rateStars, rateCount, count } = storeProfile;
