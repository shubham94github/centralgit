...
const Startups = ({ newStartups, ratedStartups, relatedStartups, isLoading }) => {
	const entityOfStartups = useMemo(() => generateEntityOfStartups(newStartups, ratedStartups, relatedStartups),
	...
	useEffect(() => {
		setSelectedStartups(entityOfStartups.find(item => item.id === selectedMenuId));
	...
	return (
		<div className='homepage-startups-container'>
			...
			<div className='startups-section'>
				{isLoading && !selectedStartups?.startups.length
					&& <LoadingOverlay/>
				}
