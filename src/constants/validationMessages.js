...
export const getMaxTextareaLengthErrMsg = (name, max) =>
  `${name} must be less than ${max || MAX_TEXTAREA_LENGTH} characters`
export const getMaxDescriptionLengthErrMsg = name =>
  `The ${name} must be less than ${MAX_DESCRIPTION_LENGTH} characters`

export const getMaxLengthErrMsg = length => `Text must be less than ${length} characters`
export const getMinLengthErrMsg = (length, fieldName = 'text') =>
  `The ${fieldName} must be more than ${length} characters.`
...
export const getMinSolutionsTextareaWordsLengthErrMsg = name =>
  `${name} must be more than ${MIN_SOLUTIONS_TEXTAREA_WORDS_LENGTH} words`
