	...
	switch (type) {
		...
		case SET_USER:
			return state.set('user', payload.user)
				...
				.set('companyAvatar', payload.companyAvatar || state.get('companyAvatar'));
		...
		case REMOVE_COMPANY_FILE:
			return state.set('companyDocuments', state.get('companyDocuments')
				.filter(doc => doc.id !== payload.fileId));
		...
		case SET_IS_EMAIL_VERIFIED:
			return state.merge({
				...
				'emailVerificationError': payload.isEmailVerified ? null : state.get('emailVerificationError'),
			});

		case SET_EMAIL_VERIFICATION_ERROR: {
			return state.set('emailVerificationError', payload.emailVerificationError);
		}
		...
		case SET_IS_PAYMENT_RECREATION_IN_PROGRESS:
			return state.set('isPaymentRecreationInProgress', payload.isPaymentRecreationInProgress);
