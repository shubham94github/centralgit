...
import { clearSessionStorage, setItemToSessionStorage } from '@utils/sessionStorage';
import { Routes } from '@routes';
import history from '../../history';
import { getEmailDomain, toColor } from '@utils';
import { getPaymentPlansWorker, onServerErrorHandler, setSnackbar } from '@ducks/common/sagas';
...
import { clearLocalStorage, getItemFromLocalStorage, setItemToLocalStorage } from '@utils/localStorage';
import {
	getChannelById,
				...
				? {
					...
					logo120: user?.retailer?.logo120?.image
						...
						: {
							...
							image: user?.retailer?.logo120?.id
								? retailerLogo120
								: '',
						},
...
function* getUserAndCompanyImages(user, rememberMe) {
	...
	const dataProfile = isStartup ? userWithImages.startup : userWithImages.retailer;
...
export function* generateAndSetListOfPermissionsWorker({ payload: { user } }) {
	if (user.role === admin || user.role === superAdmin) {
		const listOfPermissions = generateListOfPermissions(user.authority.permissions, enums.permissionTypes);
...
export function* handleSignInWorker({ payload: { accessToken, refreshToken, rememberMe } }) {
	...
	try {
		...
		if (!channels) {
			getChannelsByUserIdSaga = yield fork(getChannelsByUserIdWorker, { payload: {
				id: user.id,
...
export function* handleUploadLogoWorker({ payload }) {
	try {
		const { auth: { user, userAvatar, companyAvatar } } = yield select();
		yield setIsLoading(true);
		const [
			{ id },
			{
...
export function* handleUploadDocumentsWorker({ payload }) {
	try {
		...
		const uploadedDocuments = yield select(state => state.auth.companyDocuments);
...
export function* handleSignUpWorker({ payload: {
	data,
	isRetail,
	role,
	isMember,
...
export function* verifyEmailWorker({ payload: { token, isRetailer, isMember } }) {
	yield put({
...
export function* sendSectorsOfCompetenceWorker({ payload: { sectorsOfCompetence } }) {
...
export function* savePaymentPlanWorker({ payload: { paymentPlanId, token, discountCode, enterpriseCode } }) {
	try {
		yield setIsLoading(true);
...
export function* sendBillingInfoWorker({ payload: { billingInfo, isNeededFetchUser } }) {
...
export function* resendVerificationWorker({ payload: { isRetailer, isMember, token } }) {
	try {
		yield setIsLoading(true);
...
export function* attachPaymentMethodWorker({ payload: {
	methodId,
	isRedirect,
	isRetryPayment,
	...
	try {
		...
		const token = getItemFromLocalStorage('resendVerificationToken')
			|| user?.token;

		const userId = yield select(selectUserId) || user?.id;

		if (!!methodId) yield attachPaymentMethodToCustomer({ methodId, token, userId });

		if (isRetryPayment) {
			yield retryPaymentWorker({ payload: { id: stripeSubscriptionId, isRedirect } });
		...
		if (!isRedirect && !isRetryPayment) {
			const { auth: { user: { retailer, startup } } } = yield select();
			const customerId = retailer?.customerId || startup?.customerId;

			yield getAllCardsInfoWorker({ payload: { customerId } });
		}

		if (isRedirect) history.push(Routes.AUTH.SIGN_UP.EMAIL_VERIFICATION_PROCEED_RETAIL);
	} catch (e) {
...
export function* getCardInfoWorker({ payload: { id } }) {
	try {
		...
		const { invoiceSettings: { defaultPaymentMethod } } = yield call(getCustomer, id);
		...
		const { card: { brand, last4 }, id: paymentMethodId } = yield call(getPaymentMethod, defaultPaymentMethod);
...
export function* getAllCardsInfoWorker({ payload: { customerId } }) {
	try {
		...
		const { invoiceSettings: { defaultPaymentMethod } } = yield call(getCustomer, customerId);

		const { data = [] } = yield call(getAllPaymentMethod, customerId);

		const cardsInfo = data.map(({ card: { brand, last4 }, id: paymentMethodId }) => ({
			brand,
			last4,
			paymentMethodId,
		}));
...
export function* recreatePaymentCardWorker({ payload: { planId, paymentMethodId } }) {
	try {
		yield setIsLoading(true);
...
export function* sendDiscountCodeWorker({ payload: { discountCodeData: { code, token, type } } }) {
	try {
		yield setIsLoading(true);
		...
		const { data: discountData } = yield call(sendDiscountCode, { code, token });
		...
		if (+`${discountData.redeemBy}000` < currentDate) {
			...
			yield all([
				...
				call(getPaymentPlansWorker, { payload: { paymentPlansData: { token: user.token } } }),
			]);
...
export function* sendEnterpriseCodeWorker({ payload: { enterpriseCodeData } }) {
	try {
		...
		const { data: enterprisePlanData } = yield call(sendEnterpriseCode, enterpriseCodeData);
...
export function* onSubscriptionPaidSuccessWorker({ payload, type }) {
	...
	const isPaymentRecreationInProgress = yield select(state => state.auth.isPaymentRecreationInProgress);
