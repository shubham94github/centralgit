...
export const handleSignIn = (data, setFormError) => async dispatch => {
	try {
		...
		const { data: { access_token: accessToken, refresh_token: refreshToken } } = await signIn(data);
	...
	try {
		...
		if (!twoFaData.isEnabled2fa) {
			if (isForgotPassword) {
				...
				dispatch(handleSignIn(data, setFormError));
		} else {
...
export const checkTwoFaForgotPassword = ({ data, history, setIsPasswordSent }) => async dispatch => {
	try {
		dispatch(setIsLoading(true));
...
export const attachPaymentMethod = (data, isRedirect = true, isRetryPayment = false, stripeSubscriptionId) => ({
