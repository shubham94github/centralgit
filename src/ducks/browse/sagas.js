...
import { incTrialSearchWorker, onServerErrorHandler, setSnackbar } from '@ducks/common/sagas';
import enums from '@constants/enums';
import { trialPeriodSearchWarning, minCharactersSearchWarning, defaultFilterCategories } from '@ducks/browse/constants';
import {
...
import { replaceListOfStartups, replaceStartupProfile } from '@utils/bookmarksHelper';
...
export function* getExtremesForRangesWorker() {
	try {
		const { browse: { extremesForRanges, filterCategories } } = yield select();

		if (isEmpty(filterCategories.numberOfClients)
			|| isEmpty(filterCategories.totalFundingAmount)
			|| isEmpty(extremesForRanges)
		) {
			const extremes = isEmpty(extremesForRanges) ? yield call(getExtremesForRanges) : extremesForRanges;
			...
			yield put({
				...
				payload: {
					field: {
						numberOfClients: [extremes.numberOfClientsMin, extremes.numberOfClientsMax],
						totalFundingAmount: [extremes.totalFundingAmountMin, extremes.totalFundingAmountMax],
					},
				},
			});
		}
...
function* getLogoFilterStartup(items) {
	try {
		...
		const logos = yield all(items.map(item => call(downloadUserAvatar, item.logo120?.id)));

		const startupsWithLogos = yield all(items.map((startup, i) => downloadLogoOfFilterStartup(startup, logos[i])));
...
export function* getStartupItemsWorker() {
	try {
		...
		const { browse: { filterCategories } } = yield select();
		const lastIndexCategory = filterCategories.categoryIds?.length - 1;
		const categoryIds = lastIndexCategory >= 0
			? [filterCategories.categoryIds[lastIndexCategory]]
			: [];

		const { items, count } = yield call(getFilterStartups, { ...filterCategories, categoryIds });
...
export function* getHistoryWorker() {
	try {
		...
		const [
			{ items: searchHistory },
			{ items: savedSearchHistory },
		] = yield all ([
			call(getSearchHistory),
			call(getSavedSearchHistory),
		]);
...
export function* saveHistoryWorker({ payload: { data } }) {
	try {
		...
		const savedSearchHistoryItem = yield call(saveSearchHistory, { ...filterCategories, title });

		yield put({
			type: SET_SAVED_SEARCH_HISTORY,
			payload: { savedSearchHistory: [savedSearchHistoryItem, ...savedSearchHistory] },
		});
	} catch (e) {
		yield onServerErrorHandler(e);
	} finally {
...
export function* editHistoryWorker({ payload: { data } }) {
	try {
		...
		const changedSearchHistoryItem = yield call(editSearchHistory, { savedSearchId, title });
...
export function* removeSavedSearchWorker({ payload: { data } }) {
	try {
		...
		yield put({
			type: SET_SAVED_SEARCH_HISTORY,
			payload: { savedSearchHistory: savedSearchHistory.filter(history => history.id !== savedSearchId) },
		});
	} catch (e) {
		yield onServerErrorHandler(e);
	} finally {
