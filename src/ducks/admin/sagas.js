...
export function* handleMenuItemWorker({ payload: { itemIndex } }) {
	const { admin: { menuItems } } = yield select();
	const newMenuItems = menuItems.reduce((acc, item, i) => {
		if (i === itemIndex)
			acc.push({ ...item, isExpanded: !item.isExpanded });
		 else
			acc.push(item);
...
export function* approveUserWorker({ payload: { ids, type } }) {
	try {
		...
		const tableMeta = type === verifyUserTypes.startup
			? yield select(selectStartupsTableMeta)
			: yield select(selectRetailersTableMeta);

		if (type === verifyUserTypes.startup)
			yield call(approveStartups, ids);
...
export function* handleStartupActivationWorker({ payload: { ids, isBlocked } }) {
	try {
		...
		if (isBlocked)
			yield call(unBlockStartup, ids);
		 else
			yield call(blockStartup, ids);
...
export function* handleRetailerActivationWorker({ payload: { ids, isBlocked } }) {
	try {
		...
		if (isBlocked)
			yield call(unBlockRetailer, ids);
		 else
			yield call(blockRetailer, ids);
...
export function* getStartupProfileWorker({ payload: { id } }) {
	try {
		...
		const profile = {
			...
			avatar60: {
				...resp.avatar60,
				image: resp.avatar60?.id
					? yield call(downloadUserAvatar, resp.avatar60.id)
					: '',
			...
			startup: {
				...
				logo60: {
					...resp.startup.logo60,
					image: resp.startup.logo60?.id
						? yield call(downloadCompanyAvatar, resp.startup.logo60?.id)
						: '',
...
export function* handleRemoveDocumentWorker({ payload: { idDocument, id } }) {
	try {
		yield setIsLoading(true);
		const selectedDocuments = yield select(selectUploadedDocuments);
		const selectedDocumentsIds = selectedDocuments
			.filter(doc => doc.id !== idDocument)
			.map(doc => doc.id);
...
export function* mailingProfileWorker({ payload: { ids, isEnableMailing } }) {
	try {
		...
		if (isEnableMailing)
			yield call(enableMailing, { ids });
		else
			yield call(disableMailing, { ids });
...
export function* updateAdminNoteWorker({ payload: { data, id } }) {
	try {
		...
		const { startup: { adminNote } } = yield call(updateAdminNote, data, id);
	...
	try {
		return {
			...
			logo: {
				...item.logo,
				image: item.logo
					? yield call(downloadCategoryIcon, item.logo.id)
					: null,
			},
			areasLogo: {
				...item.areasLogo,
				image: item.areasLogo
					? yield call(downloadCategoryIcon, item.areasLogo.id)
					: null,
			},
		};
	} catch (e) {
...
export function* getReportsWorker({ payload: { meta, withLoading } }) {
	try {
		...
		const [
			...
			yield call(getAllReports, meta),
			yield call(getCountOfAllReports),
		]);
...
export function* updateBillingAddressWorker({ payload: { data } }) {
	try {
		...
		const { retailer: { id }, id: userId } = yield select(selectProfile);
					...
					? {
						...
						logo30: {
							...
							image: profileActivity.user.startup.logo30
								...
								: '',
							name: !profileActivity.user.startup.logo30?.id
								? profileActivity.user.startup?.companyShortName
								: '',
...
export function* editSubscriptionPlanWorker({ payload: {
	updatedPlan,
