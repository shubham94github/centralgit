...
export const errorChangePlanText =
  "The plan can't be applied. " +
...
export function* sendAccountInfoWorker({
...
export function* sendCompanyInfoWorker({
  payload: { companyInfo, isRetailer },
}) {
  try {
    yield setIsLoading(true);

    const saveCompanyInfo = isRetailer
      ? sendCompanyInfoRetailer
      : sendCompanyInfoStartup;
...
export function* downloadAvatarForMember(member) {
  const [avatar30, avatar60, avatar120] = yield all([
...
export function* getMembersWorker() {
  try {
    ...
    const membersWithLogos = yield all(
      members.map((member) => downloadAvatarForMember(member))
    );
...
export function* addNewMemberWorker({
...
export function* changeMemberStatusWorker({
...
export function* editMemberWorker({
...
export function* toggleIsCancelledSubscriptionWorker({
  payload: { isCancelledSubscription },
}) {
  try {
    yield setIsLoading(true);

    const activationSubscriptionApi = isCancelledSubscription
      ? subscriptionReactivate
      : subscriptionCancel;
...
export function* getPaymentPlansWorker() {
  try {
    ...
    const {
      items: paymentPlans,
      features: { items: plansFeatures },
    } = yield call(getPaymentPlans);
...
export function* sendEnterpriseCodeWorker({ payload: { enterpriseCodeData } }) {
  try {
    ...
    const enterprisePlanData = yield call(sendEnterprise, {
      code: enterpriseCodeData.code,
    });
